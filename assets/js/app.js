...
/**
 * add event on element or multiple elements;
...
let /**{String} */ apiUrl = "https://api.github.com/users/FTMFR";
...
window.updateProfile = function (profileUrl) {
    ...
    <div class="card-body">
      <div class="skeleton title-skeleton text-1"></div>
      <div class="skeleton text-skeleton text-3"></div>
    </div>
    ...
    fetchDataFromServer(profileUrl, data => {
        const {
            type,
            avatar_url,
            name,
        ...
        <ul class="profile-state">
            <li class="state-item"><span class="body"> ${public_repos} </span> Repos</li>

            <li class="state-item"><span class="body"> ${numberToKilo(followers)} </span> Followers</li>

            <li class="state-item"><span class="body"> ${numberToKilo(following)} </span> Following</li>
        </ul>

        <div class="footer">
            <p class="copyright">&copy; ${name}</p>
        </div>
        `;
...
let /**{Array} */ forkedRepository = [];
const updateRepositories = function () {
    fetchDataFromServer(`${repoUrl}?sort=created&per_page=12`, function (data) {
        $repoPanel.innerHTML = `
        <h2 class="sr-only">Repositories</h2>`;
        forkedRepository = data.filter(item => /**{Boolean} */ item.fork);
...
const /**{NodeElement} */ $forkedPanel = document.querySelector('[data-fork-panel]');
...
const updateForkRepo = function () {
    ...
    console.log(forkedRepository.length)
    if (forkedRepository.length) {
        for (const repo of forkedRepository) {
            const {
                name,
                html_url,
                description,
            ...
            const /**{NodeElement} */ $forkeCard = document.createElement('article');
            $forkedPanel.classList.add('card', 'repo-card');
            $forkedPanel.innerHTML = `
            <div class="card-body">
            <a href="${html_url}" target="_blank" class="card-title">
              <h3 class="title-3">${name}</h3>
            ...
            $forkedPanel.appendChild($forkeCard);
        }
    } else {
...
const /**{NodeElement} */ $followerPanel = document.querySelector('[data-followers-panel]');
...
const updateFollowers = function () {
    ...
    fetchDataFromServer(followerUrl, function (data) {
        $followerPanel.innerHTML = `
        ...
        if (data.length) {
            for (const item of data) {
                ...
                const /**{NodeElement} */ $followCard = document.createElement('article');
                ...
                $followerPanel.appendChild($followCard);
            }
        } else {
            $followerPanel.innerHTML = `
            <div class="error-content">
...
$followerTabBtn.addEventListener('click', updateFollowers);
...
//     fetchDataFromServer(followingUrl, function (data) {
//         $followingPanel.innerHTML = `
